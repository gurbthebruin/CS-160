...
; 
; Returns the index of the propositional variable that represents the constraint
; using variable index = (n-1)*k + c
;
...
; 
; Returns a clause that represents the constraint
...
; Helper function to check pairs such that node n gets at most one color from the set

(defun help_max (n a b k) 
	(if (<= b k)
		(cons (list (- (node2var n a k)) (- (node2var n b k))) (help_max n a (+ b 1) k))
		nil
	)
)
...
(defun at-most-one-color (n c k)
	(if (<= c k)
		(append (help_max n c (+ c 1) k) (at-most-one-color n (+ c 1) k))
		nil
	)
)
...
(defun help_check (e c k)
  (cond ((> c k) nil)
        (t (append (list (list (- (node2var (first e) c k)) (- (node2var (second e) c k)))) (help_check e (+ c 1) k)))
  )
)
